...
public class      PoolGroupLinkCollector
   ...
   private void collectionFailed( Object obj ){
      _collectionState = 0 ;
      System.err.println("Problem in collection PoolInfos : "+obj ) ;
      fireAction(obj);
   }
   
   private void askForPoolCost(){
      if(_debug)System.out.println("Asking for pool cost") ;
      try{
   ...
   private void askForLinkGroups(){
      if(_debug)System.out.println("Asking for link groups") ;
      try{
	 _connection.sendObject( "PoolManager" ,
                        	 "psu ls -l linkGroup" ,
                        	 this ,
   ...
   private void askForPoolsOfGroup( String poolGroup ){
      if(_debug)System.out.println("Asking for pools of "+poolGroup) ;
      try{
	 _connection.sendObject( "PoolManager" ,
                        	 poolGroup.equals("*") ?
				 "psux ls pool" :
   ...
   private void askForPoolGroups(){
      if(_debug)System.out.println("Asking for all pool groups") ;
      try{
	 _connection.sendObject( "PoolManager" ,
                        	 "psux ls pgroup" ,
                        	  this ,
   ...
   private void askForLinkList(){
      System.out.println("Asking for link list") ;
      try{
	 _connection.sendObject( "PoolManager" ,
                        	 "psux ls link" ,
                        	  this ,
   ...
   private Map scanLinkGroupOutput(String in ) throws IllegalArgumentException {
	 ...
	 ArrayList list          = new ArrayList() ;
	   ...
	   if( str.equals("]") )break ;
	   if(_debug)System.out.println("Token : >"+str+"<");
	   switch(i){
   ...
   public void domainAnswerArrived( Object obj , int subid ){
      ...
      if(_debug)System.out.println( "Answer ("+subid+") : "+obj.getClass().getName()+" "+obj.toString() ) ;
	 ...
	 try{
	    ...
	    if(_debug)
	    System.out.println("PoolGroupLinkCollector CostModulePoolInfoTable : "+_costTable);
	 }catch(Exception ee ){
	    ee.printStackTrace();
	 ...
	 try{
	    ...
	    if(_debug)System.out.println("PoolGroupLinkCollector LinkGroupMap : "+_linkGroupHash);
	 }catch(Exception ee ){
	    ee.printStackTrace();
	    collectionFailed(ee.getMessage()) ; 
	    return ;
	 ...
	 if(_debug)System.out.println(""+array.length+" Pool Groups arrived" ) ;
	 ...
	 Object [] array = (Object [])obj ;
	 if(_debug)System.out.println(""+array.length+" Pools arrived of *" ) ;
	 ...
	 if(_debug)System.out.println(""+array.length+" Pools arrived of "+expectedPoolGroup ) ;
	 ArrayList list  = new ArrayList() ;
	 ...
	 if( _selectedPool >= _poolGroups.size() ){
	    ...
	    //set.addAll( _poolGroups ) ;
            ...
            if( _collectLinks ){
               if(_debug)System.out.println("JPoolGroupList : switching to link, ask for link list");
	 ...
	 ArrayList list  = new ArrayList() ;
         ...
         if(_debug)System.out.println("Link list arrived : "+list) ;
         ...
         if( res.length < 9 ){
             System.err.println("Not enought slots in reply on 'psux ls link <link>'");
	 ...
	 entry.p2pPref   = ((Integer)res[7]).intValue() ;
         ...
         if(_debug)System.out.println("Got link entry : "+entry ) ;
   ...
   private synchronized void finishLinkCollection(){
      ...
      for( Iterator it = _linkList.iterator() ; it.hasNext() ; ){
          ...
          //System.out.println("Link name : "+entry.name);
          ArrayList pools = new ArrayList() ;
          
          for( int i = 0 ; i < entry.pools.length ; i++ ){
	      //System.out.println("    "+entry.pools[i]);
	     ...
	     //System.out.println("        group : "+poolGroupName+" "+list);
          }
	  //System.out.println("       resolved : "+pools);
   ...
   public void connectionOpened( DomainConnection connection ){
      if(_debug)System.out.println("PoolGroupLinkCollector : Connection opened");
      //collectData();
   }
   public void connectionClosed( DomainConnection connection ){
      if(_debug)System.out.println("PoolGroupLinkCollector : Connection closed" ) ;
   }
   public void connectionOutOfBand( DomainConnection connection, Object obj ){
      if(_debug)System.out.println("PoolGroupLinkCollector : Connection connectionOutOfBand "+obj ) ;
   }
